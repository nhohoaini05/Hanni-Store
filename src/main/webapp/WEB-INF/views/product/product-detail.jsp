			...
			<div id="MainContent" class="main-content" role="main">
				...
				<div id="ProductSection-product-template"
					...
					<div class="product-single">
								...
								<div class="product-single__meta">
									...
									<p class="product-single__price product-single__price-product-template">
										<ul class="list-group list-group-flush">
											<li class="list-group-item alert alert-danger" style="font-size: 30px;" role="alert" id="productInfo_${product.product.id}"></li>
										</ul>	
										
										<script>
	                                        // Lấy phần tử HTML chứa thông tin sản phẩm
	                                        var listItem = document.getElementById("productInfo_${product.product.id}");
	
	                                     	// Lấy giá và giảm giá từ đối tượng d
	                                        var price_${product.product.id} = ${product.product.price};
	                                        var discount_${product.product.id} = ${product.product.discount};
	
	                                        // Tính giá đã giảm và giá gốc
	                                        var discountedPrice_${product.product.id} = price_${product.product.id} * (1 - discount_${product.product.id} / 100);
	                                        var originalPrice_${product.product.id} = price_${product.product.id};
	
	                                        // Tạo nội dung HTML để hiển thị
	                                        var htmlContent_${product.product.id} = "";
	                                        if (discount_${product.product.id} > 0) {
	                                            htmlContent_${product.product.id} += "<span style='text-decoration: line-through; font-weight-light; font-size: 15px;'>đ" + originalPrice_${product.product.id}.toLocaleString() + "</span> ";
	                                            htmlContent_${product.product.id} += "<span class='font-size: 30px; font-weight-bold;'>đ" + discountedPrice_${product.product.id}.toLocaleString() + "</span> ";
	                                            htmlContent_${product.product.id} += "<span class='badge badge-pill badge-primary'> - " + discount_${product.product.id} + "%</span>";
	                                        } else {
	                                            htmlContent_${product.product.id} += "<span>Đ" + originalPrice_${product.product.id}.toLocaleString() + "</span>";
	                                        }
	
	                                        // Gán nội dung HTML vào phần tử
	                                        listItem.innerHTML = htmlContent_${product.product.id};
	                                    </script>
									</p>
						...
						<div
							style="width: 800px; height: 150px; margin: auto; border: 1px solid rgb(246, 246, 246);"
							class="comment">
							<h5
								style="padding-left: 20px; margin: 0; position: relative; top: 10px; color: #000000; font-weight: 600;">Bình Luận</h5>
							<div style="padding:20px;" class="commentip">
								<input type="text" placeholder="Gửi đánh giá của bạn vào đây">
							</div>
							<div style="padding-left: 20px; margin: 0;">
								<button
									style="cursor: pointer; width: 100px;height: 40px;  background: #ff708e; color: aliceblue;"
