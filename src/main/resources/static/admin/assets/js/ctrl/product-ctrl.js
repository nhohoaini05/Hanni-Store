...
app.controller("product-ctrl", function ($scope, $http) {
    ...
    $scope.init = function () {
        //LOAD DB//dlsp
        $http.get(urlProduct).then(resp => {//thực thi lệnh resp
            $scope.db = resp.data;//lưu trử sp vào scope
            $scope.db.products.forEach(p => {
                p.createDate = new Date(p.createDate);//duyệt sp trong mảng p sau đó chuyển đổi
            });
        }).catch(error => {
            alert("Load data fail");
        });
        $scope.start = 0;//đặt biến start hiển thị sp đầu tiên
    }
    $scope.next = function(){
